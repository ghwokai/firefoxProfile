...
var site = {
	...
	'codeRule':/(?:提取码|密码|访问码?|艾码)[:： ]?\s*([a-z\d]{4})/i
};
	...
	$('A').click(function(){
		...
		if(HostExp.test(this.href)){
			if(site['codeRule'].test(target.textContent)){
				...
				pw=target.textContent.match(site['codeRule'])[1];
				target.href+='#'+pw;
			} else if(target.nextSibling&&site['codeRule'].test(target.nextSibling.textContent)){
				console.log('密码在超链接后面的兄弟元素中');
				eventNext = target.nextSibling.textContent.trim();
				pw=eventNext.match(site['codeRule'])[1];
				if(!/#/i.test(target.href)) target.href+='#'+pw;
			} else if(site['codeRule'].test(target.parentNode.textContent)){
				console.log('从父对象中查找密码');
				if(!/#/i.test(target.href)) target.href+='#'+target.parentNode.textContent.match(site['codeRule'])[1];
			}
		}
	})
}
