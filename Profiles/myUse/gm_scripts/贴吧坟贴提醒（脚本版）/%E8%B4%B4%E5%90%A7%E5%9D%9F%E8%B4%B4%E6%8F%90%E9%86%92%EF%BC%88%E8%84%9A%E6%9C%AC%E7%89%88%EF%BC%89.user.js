...
// @name       贴吧坟贴提醒（脚本版）
// @version    0.7
// @description  修改自：http://tieba.baidu.com/p/1748230170 原扩展作者864907600cc，修改者h573980998 自定义by绯色起源
// @homepage    https://greasyfork.org/scripts/367/
...
var tietime=(JSON.parse(document.getElementsByClassName('l_post')[0].getAttribute('data-field')).content.date.replace(/-/g,"/")).substr(0,10);
...
if(x>pd_this_ft_t()){
run('此贴已存在'+showtext+'('+x+'天)，已为坟贴，脚本已经关闭回复！ㄟ(￣▽￣ㄟ)' );
check=1;//是坟贴
}}
function run(xmx){
...
GM_addStyle('.jiangyou,.t_qreply{display:none;}');//将大花猫的队形按钮也屏蔽,神来一句也闪吧
